...
export const ArticleRows = ({item}) => {
  ...
  // console.log({columns})
  return (
    <section className={`grid ${item.width === "100%" && "grid-cols-1"} ${item.width === "50%-50%" && "grid-cols-1 md:grid-cols-2 gap-4"} ${item.width === "33%-33%-33%" && "grid-cols-1 lg:grid-cols-3"} ${item.width === "25%-25%-25%-25%" && "grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4"}`}>
      {
        columns.map( column => {
          // console.log({column})
